...
export default function PitchProject(props: ProjectDetailProps) {
  ...
  return (
    <main className={styles.page} dir={dir} data-layout="deck">
      <nav className={styles.nav} aria-label="Project"><Link href={props.variantPath || "/" + locale}>← {locale === "ar" ? "رجوع" : "Back"}</Link><span>Pitch Slides · 10</span></nav>
      <section className={styles.hero}>
        <div className={styles.copy}><p className={styles.eyebrow}>Neo-Brutalist Pitch Deck</p><h1>{pick(project.title, locale)}</h1><p>{pick(project.description, locale) || pick(project.summary, locale)}</p></div>
