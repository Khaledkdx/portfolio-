...
export default function BroadcastStudioProject(props: ProjectDetailProps) {
  ...
  return (
    <main className={styles.page} dir={dir} data-layout="broadcast">
      <nav className={styles.nav} aria-label="Project"><Link href={props.variantPath || "/" + locale}>← {locale === "ar" ? "رجوع" : "Back"}</Link><span>Live Broadcast · 28</span></nav>
      <section className={styles.hero}>
        <div className={styles.copy}><p className={styles.eyebrow}>Broadcast Studio</p><h1>{pick(project.title, locale)}</h1><p>{pick(project.description, locale) || pick(project.summary, locale)}</p></div>
