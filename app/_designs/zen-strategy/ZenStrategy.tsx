...
import styles from "./zen-strategy.module.css";

export default function ZenStrategy({ content, locale, design, preview, variantPath }: DesignProps) {
  const projects = publishedProjects(content);
  const languageUrl = languageHref({ content, locale, design, preview, variantPath });
  return (
    <main className={styles.page} dir={locale === "ar" ? "rtl" : "ltr"} data-layout="zen">
      <a className={styles.skip} href="#work">Skip to work</a>
      <nav className={styles.nav} aria-label="Portfolio">
        <Link href={variantPath || "/" + locale} className={styles.brand}><span>K/18</span><b>{content.profile.name}</b></Link>
        <div>
      ...
      <section className={styles.hero} aria-labelledby="hero-title">
        <div className={styles.heroCopy}>
          <p className={styles.eyebrow}>Ink Strategy · calm ink and whitespace advisory</p>
        ...
        <div className={styles.heroVisual} aria-label={content.profile.name}>
          ...
          <div className={styles.signature} aria-hidden="true"><span>18</span><b>Zen Strategy</b></div>
        </div>
      </section>
