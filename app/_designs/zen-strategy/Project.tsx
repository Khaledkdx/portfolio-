...
export default function ZenStrategyProject(props: ProjectDetailProps) {
  const { content, locale, project } = props;
  const { images, links, previous, next, projectUrl, dir } = projectDetailData(props);
  const cover = images[0];
  return (
    <main className={styles.page} dir={dir} data-layout="zen">
      <nav className={styles.nav} aria-label="Project"><Link href={props.variantPath || "/" + locale}>← {locale === "ar" ? "رجوع" : "Back"}</Link><span>Ink Strategy · 18</span></nav>
      <section className={styles.hero}>
        <div className={styles.copy}><p className={styles.eyebrow}>Zen Strategy</p><h1>{pick(project.title, locale)}</h1><p>{pick(project.description, locale) || pick(project.summary, locale)}</p></div>
