...
export default function ModularCubesProject(props: ProjectDetailProps) {
  ...
  return (
    <main className={styles.page} dir={dir} data-layout="cubes">
      <nav className={styles.nav} aria-label="Project"><Link href={props.variantPath || "/" + locale}>← {locale === "ar" ? "رجوع" : "Back"}</Link><span>Voxel Builder · 13</span></nav>
      <section className={styles.hero}>
        <div className={styles.copy}><p className={styles.eyebrow}>Modular Cubes</p><h1>{pick(project.title, locale)}</h1><p>{pick(project.description, locale) || pick(project.summary, locale)}</p></div>
