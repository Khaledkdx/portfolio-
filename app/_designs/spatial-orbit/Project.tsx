...
export default function SpatialOrbitProject(props: ProjectDetailProps) {
  ...
  return (
    <main className={styles.page} dir={dir} data-layout="orbit">
      <nav className={styles.nav} aria-label="Project"><Link href={props.variantPath || "/" + locale}>← {locale === "ar" ? "رجوع" : "Back"}</Link><span>Spatial Orbit · 12</span></nav>
      <section className={styles.hero}>
        <div className={styles.copy}><p className={styles.eyebrow}>Spatial 3D Orbit</p><h1>{pick(project.title, locale)}</h1><p>{pick(project.description, locale) || pick(project.summary, locale)}</p></div>
